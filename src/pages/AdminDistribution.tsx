...
const empty = {
  ...
  category: "Général", commission_percent: 15, is_active: true,
  images: [] as string[],
};
...
const AdminDistribution = () => {
  ...
  const [editing, setEditing] = useState<DP | null>(null);
  const [form, setForm] = useState<typeof empty>(empty);
  const [uploading, setUploading] = useState(false);

  const load = async () => {
    const [p, o] = await Promise.all([
      supabase.from("distribution_products" as any).select("*").order("created_at", { ascending: false }),
      supabase.from("distribution_orders" as any).select("*").order("created_at", { ascending: false }),
    ]);
  ...
  const openEdit = (p: DP) => {
    ...
    setForm({
      ...
      stock: p.stock, category: p.category || "Général",
  ...
  const uploadImages = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      const path = `distribution/${Date.now()}-${f.name.replace(/[^a-z0-9.]/gi, "_")}`;
  ...
  const save = async () => {
    if (!form.name || form.price <= 0) { toast.error("Nom et prix requis"); return; }
    const res = editing
      ? await supabase.from("distribution_products" as any).update(form).eq("id", editing.id)
      : await supabase.from("distribution_products" as any).insert(form);
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from("distribution_products" as any).delete().eq("id", id);
    if (error) { toast.error(error.message); return; }
    toast.success("Supprimé"); load();
  };

  const setStatus = async (id: string, status: string) => {
    const { error } = await supabase.from("distribution_orders" as any).update({ status }).eq("id", id);
    if (error) { toast.error(error.message); return; }
    toast.success(`Statut : ${status}`); load();
  };
    ...
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground font-body text-sm">Gérez les produits de distribution</p>
        </div>
        {tab === "products" && (
      ...
      {tab === "products" && (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map(p => (
            <div key={p.id} className="card-elevated">
              ...
              <div className="text-sm font-body space-y-1 mb-3">
                <p>Prix : <strong>{Number(p.price).toLocaleString("fr-FR")} F</strong></p>
          ...
          {products.length === 0 && <p className="text-muted-foreground font-body col-span-full text-center py-8">Aucun produit.</p>}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 overflow-auto">
          <div className="bg-card rounded-2xl max-w-2xl w-full p-6 my-8">
            ...
            <div className="grid sm:grid-cols-2 gap-3 font-body text-sm">
              <input className="input-field sm:col-span-2" placeholder="Nom" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
              ...
              <label className="text-xs">Prix (F)
              ...
              <label className="text-xs">Commission (%)
