...
const StandPage = () => {
  ...
  const handleBuyProduct = async (e: React.FormEvent) => {
    ...
    setBuyProduct(null); setSubmitting(false); loadData();
  };
      ...
      <header className="border-b border-border bg-card sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-heading text-lg font-bold text-foreground">🏢 Stand Partenaires</span>
          </div>
            ...
            <div key={partner.id} className="card-elevated hover:shadow-lg transition-shadow cursor-pointer" onClick={() => setSelectedPartner(partner)}>
              {partner.image1_url && (
                <div className="relative -mx-4 -mt-4 mb-4 rounded-t-xl overflow-hidden">
                  <img src={partner.image1_url} alt="" className="w-full h-32 object-cover" loading="lazy" />
                </div>
              )}
              <div className="flex items-center gap-3 mb-4">
                {partner.logo_url ? (
                  <img src={partner.logo_url} alt={partner.name} className="w-14 h-14 rounded-xl object-cover border border-border" loading="lazy" />
                ) : (
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {partner.phone && (
                  <a href={`tel:${partner.phone}`} onClick={e => e.stopPropagation()}
                    className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-accent/10 text-accent-foreground text-xs font-semibold hover:bg-accent/20 transition-colors">
        ...
        {selectedPartner && (
          <div className="fixed inset-0 bg-foreground/50 flex items-center justify-center z-50 p-4" onClick={() => setSelectedPartner(null)}>
            <div className="bg-card rounded-xl border border-border p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              {selectedPartner.image1_url && <img src={selectedPartner.image1_url} alt="" className="w-full h-40 rounded-lg object-cover mb-4" loading="lazy" />}
              <div className="flex items-center gap-4 mb-4">
                {selectedPartner.logo_url ? (
                  <img src={selectedPartner.logo_url} alt={selectedPartner.name} className="w-16 h-16 rounded-xl object-cover border border-border" loading="lazy" />
                ) : (
                  ...
                  <div className="space-y-3">
                    {partnerProducts(selectedPartner.id).map(prod => (
                      <div key={prod.id} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50">
                        {prod.images?.[0] && <img src={prod.images[0]} alt="" className="w-16 h-16 rounded-lg object-cover" loading="lazy" />}
              ...
              <button onClick={() => setSelectedPartner(null)} className="w-full py-2.5 rounded-lg border border-input text-muted-foreground font-body text-sm hover:bg-secondary">Fermer</button>
            </div>
          </div>
        )}
