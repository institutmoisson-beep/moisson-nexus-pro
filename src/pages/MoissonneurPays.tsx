...
import {
  Globe, Users, Search, Ban, CheckCircle, Eye, X,
  ArrowLeft, LogOut, History, Activity, RefreshCw,
  UserCheck, UserX, AlertTriangle, TrendingUp, Filter
} from "lucide-react";
...
const MoissonneurPays = () => {
  const { user, signOut } = useAuth();
  const navigate = useNavigate();
  const [myProfile, setMyProfile] = useState<any>(null);
  const [assignedCountry, setAssignedCountry] = useState<string>("");
  ...
  const loadAll = async () => {
    ...
    // Get assigned country from staff_roles metadata or profiles
    const { data: roleData } = await supabase
      .from("staff_roles")
      .select("*")
      .eq("user_id", user!.id)
      .eq("role", "moissonneur_pays")
      .single();

    const country = (roleData as any)?.assigned_country || profile?.country || "";
    setAssignedCountry(country);
  ...
  const getNetworkCount = async (userId: string) => {
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-primary font-body font-semibold">{assignedCountry || "Pays non assigné"}</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
      ...
      <div className="container mx-auto px-4 py-6">
        {!assignedCountry ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-heading font-bold text-foreground mb-2">Aucun pays assigné</h2>
            <p className="text-muted-foreground font-body">L'administrateur doit vous assigner un pays pour activer votre rôle.</p>
          </div>
        ) : (
          <>
            {/* STATS */}
            ...
            <div className="card-elevated overflow-hidden">
              <div className="p-4 border-b border-border flex items-center justify-between">
                <h2 className="font-heading font-bold text-foreground">
                  Membres de {assignedCountry} ({filteredMembers.length})
                </h2>
              </div>
                  ...
                  <tbody>
                    {filteredMembers.map(member => (
                      <tr key={member.id} className={`border-b border-border/50 hover:bg-secondary/20 transition-colors ${member.is_suspended ? "opacity-60" : ""}`}>
                        ...
                        <td className="py-3 px-4 text-muted-foreground">{member.city || "—"}</td>
                    ...
                    {filteredMembers.length === 0 && (
                      <tr>
                        <td colSpan={7} className="py-12 text-center text-muted-foreground font-body">
                          {search ? "Aucun résultat pour cette recherche" : `Aucun membre en ${assignedCountry}`}
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="bg-card w-full md:max-w-3xl max-h-[90vh] md:rounded-2xl rounded-t-2xl overflow-hidden flex flex-col border border-border shadow-2xl"
              ...
              <div className="flex gap-1 bg-secondary p-1 mx-5 mt-4 rounded-xl flex-shrink-0">
                {[
                  ...
                  { key: "activity", label: "📦 Commandes" },
                ].map(tab => (
                  <button
                    key={tab.key}
                    onClick={() => setActiveTab(tab.key as any)}
                    className={`flex-1 px-3 py-2 rounded-lg text-xs font-body font-semibold transition-all ${
                ...
                {activeTab === "overview" && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { label: "Solde", value: `${Number(selectedMember.wallet_balance).toLocaleString("fr-FR")} FCFA`, color: "text-primary" },
                        ...
                        { label: "MLM", value: selectedMember.is_mlm_active ? "Actif" : "Inactif", color: selectedMember.is_mlm_active ? "text-harvest-green" : "text-muted-foreground" },
                        { label: "Carrière", value: selectedMember.career_level?.replace(/_/g, " "), color: "text-gold" },
                      ].map((item, i) => (
                    ...
                    <div className="grid md:grid-cols-2 gap-3 text-sm font-body">
                      ...
                      <div className="bg-secondary/50 rounded-lg p-3 space-y-1.5">
                        ...
                        <p><span className="text-muted-foreground">Code parrainage :</span> <span className="font-mono font-semibold ml-1">{selectedMember.referral_code}</span></p>
                      </div>
                    </div>
                  </div>
                )}
...
export default MoissonneurPays;
